...
#            de l'incidence du paludisme au Bénin
# ==============================================================================
...
# Chemin vers le shapefile du Bénin
gadm_path <- "data/gadm41_BEN_shp"
...
if(is.null(gadm_path) || !file.exists(gadm_path)) {
  ...
  if(length(shp_candidates) == 0) {
    stop("Aucun fichier .shp trouvé dans 'data/gadm41_BEN_shp'.")
  }
...
# Calculer la moyenne de l'incidence pour chaque année (sur tout le Bénin)
# On ignore les NA (océan et zones hors Bénin)
...
p_evolution <- ggplot(df_stats, aes(x = annee, y = incidence_moyenne)) +
  ...
  labs(
    title = "Évolution de l'incidence du paludisme au Bénin (2000-2024)",
...
p_carte <- ggplot() +
  ...
  coord_sf() +
  labs(
    title = paste("Incidence du paludisme au Bénin -", annee_a_cartographier),
