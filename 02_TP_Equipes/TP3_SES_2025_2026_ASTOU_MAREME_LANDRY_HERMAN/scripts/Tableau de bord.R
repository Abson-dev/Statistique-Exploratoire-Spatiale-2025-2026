...
# =============================================================================
# FONCTIONS CORRIGÉES POUR L'IDENTIFICATION DES ZONES DÉFAVORISÉES
# =============================================================================

identifier_zones_defavorisees_corrige <- function(analyses) {
  cat("Identification des zones défavorisées (version corrigée)...\n")
  
  if (!is.null(analyses$accessibilite_sante) && !is.null(analyses$accessibilite_education)) {
    
    # Vérifier que les deux datasets ont les mêmes villages
    villages_sante <- analyses$accessibilite_sante$name
    villages_education <- analyses$accessibilite_education$name
    
    # Trouver l'intersection des villages communs
    villages_communs <- intersect(villages_sante, villages_education)
    
    if (length(villages_communs) == 0) {
      cat("❌ Aucun village commun entre les données de santé et d'éducation\n")
      return(NULL)
    }
    
    cat("📊 Villages communs trouvés:", length(villages_communs), "\n")
    
    # Filtrer les données pour ne garder que les villages communs
    sante_filtre <- analyses$accessibilite_sante %>% 
      dplyr::filter(name %in% villages_communs)
    
    education_filtre <- analyses$accessibilite_education %>% 
      dplyr::filter(name %in% villages_communs)
    
    # CORRECTION : Utiliser l'extraction directe des colonnes sans select()
    education_data <- data.frame(
      name = education_filtre$name,
      distance_ecole_km = education_filtre$distance_ecole_km,
      categorie_access_ecole = education_filtre$categorie_access_ecole,
      stringsAsFactors = FALSE
    )
    
    # Créer un dataframe combiné
    zones_defavorisees <- sante_filtre %>%
      dplyr::left_join(education_data, by = "name") %>%
      dplyr::mutate(
        # Score basé sur l'accès aux services
        score_sante = dplyr::case_when(
          distance_hopital_km <= 10 ~ 1,
          distance_hopital_km <= 25 ~ 2,
          distance_hopital_km <= 50 ~ 3,
          TRUE ~ 4
        ),
        score_education = dplyr::case_when(
          distance_ecole_km <= 2 ~ 1,
          distance_ecole_km <= 5 ~ 2,
          distance_ecole_km <= 10 ~ 3,
          TRUE ~ 4
        ),
        score_total = score_sante + score_education,
        
        # Catégorisation
        niveau_defavorisation = dplyr::case_when(
          score_total >= 7 ~ "Très défavorisé",
          score_total >= 5 ~ "Défavorisé",
          score_total >= 3 ~ "Modérément défavorisé",
          TRUE ~ "Bien desservi"
        ),
        
        # Priorité d'intervention
        priorite_intervention = dplyr::case_when(
          niveau_defavorisation == "Très défavorisé" ~ "Priorité maximale",
          niveau_defavorisation == "Défavorisé" ~ "Priorité élevée",
          niveau_defavorisation == "Modérément défavorisé" ~ "Priorité moyenne",
          TRUE ~ "Priorité faible"
        )
      )
    
    # Statistiques
    cat("📊 Répartition des zones défavorisées:\n")
    table_defavorise <- table(zones_defavorisees$niveau_defavorisation)
    for (niveau in names(table_defavorise)) {
      cat("   ", niveau, ":", table_defavorise[niveau], "villages\n")
    }
    
    return(zones_defavorisees)
  } else {
    cat("❌ Données d'accessibilité santé ou éducation manquantes\n")
    return(NULL)
  }
}

# =============================================================================
# FONCTION POUR CRÉER LA CARTE DES ZONES DÉFAVORISÉES (CORRIGÉE)
# =============================================================================

creer_carte_zones_defavorisees_corrige <- function(donnees, analyses) {
  cat("Création de la carte des zones défavorisées (version corrigée)...\n")
  
  zones_defavorisees <- identifier_zones_defavorisees_corrige(analyses)
  
  if (!is.null(zones_defavorisees)) {
    
    # Palette pour les niveaux de défavorisation
    pal_defavorise <- leaflet::colorFactor(
      palette = c("#1a9641", "#a6d96a", "#fdae61", "#d7191c"),
      domain = zones_defavorisees$niveau_defavorisation
    )
    
    # Ajuster la taille des points selon la population
    zones_defavorisees <- zones_defavorisees %>%
      dplyr::mutate(
        radius = dplyr::case_when(
          is.na(population_estimee) ~ 6,
          population_estimee < 100 ~ 4,
          population_estimee < 1000 ~ 6,
          population_estimee < 5000 ~ 8,
          TRUE ~ 10
        )
      )
    
    carte <- leaflet::leaflet() %>%
      leaflet::addTiles(group = "Carte standard") %>%
      leaflet::addProviderTiles(leaflet::providers$CartoDB.Positron, group = "Carte claire") %>%
      
      # Ajouter les zones défavorisées
      leaflet::addCircleMarkers(
        data = zones_defavorisees,
        group = "Zones défavorisées",
        radius = ~radius,
        color = ~pal_defavorise(niveau_defavorisation),
        fillOpacity = 0.8,
        stroke = TRUE,
        weight = 1,
        popup = ~paste(
          ...
          "<p style='margin: 5px 0;'><b>🏫 Distance école:</b> ", 
          round(distance_ecole_km, 1), " km</p>",
          "<p style='margin: 5px 0;'><b>📊 Niveau de défavorisation:</b> ", 
          as.character(niveau_defavorisation), "</p>",
          "<p style='margin: 5px 0;'><b>🎯 Priorité d'intervention:</b> ", 
          as.character(priorite_intervention), "</p>",
          "</div>",
          "</div>"
        )
      ) %>%
      ...
      leaflet::addLegend(
        position = "bottomright",
        pal = pal_defavorise,
        values = zones_defavorisees$niveau_defavorisation,
        title = "Niveau de Défavorisation",
        opacity = 0.8,
        group = "Zones défavorisées"
      ) %>%
      ...
      leaflet::addLayersControl(
        baseGroups = c("Carte standard", "Carte claire"),
        overlayGroups = c("Zones défavorisées"),
        options = leaflet::layersControlOptions(collapsed = FALSE)
      ) %>%
      
      leaflet::addMiniMap(toggleDisplay = TRUE) %>%
      leaflet::addScaleBar(position = "bottomleft") %>%
      # CORRECTION : Utiliser la fonction correcte pour le contrôle plein écran
      leaflet.extras::addFullscreenControl()
    
    # Sauvegarder la carte
    htmlwidgets::saveWidget(carte, file = file.path(outputs_folder, "carte_zones_defavorisees.html"))
    cat("✅ Carte des zones défavorisées sauvegardée\n")
...
carte_zones_defavorisees <- creer_carte_zones_defavorisees_corrige(donnees_completes, analyses_politiques)
...
cat("   • 🎯 carte_zones_defavorisees.html (Zones prioritaires)\n")
...
cat("   • 🎯 Identification corrigée des zones défavorisées\n")
