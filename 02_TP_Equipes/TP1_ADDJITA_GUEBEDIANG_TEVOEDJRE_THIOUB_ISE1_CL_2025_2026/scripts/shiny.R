...
# Shapefile
shp_files <- list.files("data/gadm", pattern = "\\.shp$", full.names = TRUE, recursive = TRUE)
...
ui <- fluidPage(
  titlePanel(
    div(
      ...
      h2("🦟 Paludisme au Sénégal - Visualisation Interactive"),
        ...
        tabPanel(
          ...
          div(
            ...
            p("Cette application visualise l'évolution de l'incidence du paludisme 
              (Plasmodium falciparum) au Sénégal sur plusieurs années."),
...
server <- function(input, output, session) {
  ...
  output$download_png <- downloadHandler(
    ...
    content = function(file) {
      ...
      plot(r, 
           main = paste("Incidence du Paludisme au Sénégal -", annees[idx]),
